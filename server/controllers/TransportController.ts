...
export class TransportController {
  public static async searchTrains(req: Request, res: Response): Promise<void> {
    try {
      ...
      const results = await TransportService.searchTrains({ origin, destination, date });
      res.json({
        success: true,
  ...
  public static async searchBuses(req: Request, res: Response): Promise<void> {
    try {
      ...
      const results = await TransportService.searchBuses({ origin, destination, date });
      res.json({
        success: true,
        count: results.length,
        data: results
  ...
  public static async searchFlights(req: Request, res: Response): Promise<void> {
    try {
      ...
      const results = await TransportService.searchFlights({ origin, destination, date });
      res.json({
        success: true,
        count: results.length,
        data: results
